      ...
      {activeTab === "board" ? (
        ...
        <div>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={Object.entries(funnel).map(([stage, count]) => ({ stage, count }))}>
              <XAxis dataKey="stage" />
              <YAxis />
              <Bar dataKey="count" fill="#B8502F" />
            </BarChart>
          </ResponsiveContainer>

          {timeInStage.map((gap, index) => (
            <div key={index}>
              {gap.from_stage} → {gap.to_stage}: {gap.duration_days} days
            </div>
          ))}
        </div>
      )}
